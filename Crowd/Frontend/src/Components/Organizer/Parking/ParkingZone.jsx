...
const getColorsByValue = (v) =>
  v >= 70 ? { progress:"#22c55e", trail:"rgba(34,197,94,0.20)" }
...
const ProgressDonut = ({ value=70, subtitle="Available", size=220 }) => {
  ...
  const gradId = useId();
  const data = [{ name:"Progress", value }, { name:"Remaining", value: Math.max(0, 100-value)}];
  return (
      ...
      <ResponsiveContainer width="100%" height="100%">
        <PieChart>
          ...
          <Pie data={data} cx="50%" cy="50%" innerRadius="62%" outerRadius="80%"
               startAngle={90} endAngle={-270} dataKey="value" isAnimationActive animationDuration={900} stroke="none">
  ...
  return (
    <div className="rounded-2xl border border-white/10 bg-white/5 backdrop-blur-md p-5 hover:bg-white/10 hover:shadow-xl transition-all">
      ...
      <div className="flex justify-center">
        <ProgressDonut value={Math.max(0, Math.min(100, percent))} subtitle="Available" size={220} />
      </div>
      ...
      <div className="mt-5 grid grid-cols-3 gap-3">
        ...
        <div className="rounded-xl bg-white/5 border border-white/10 p-3 text-center">
          <div className="text-2xl font-bold text-white">{Math.max(total - available, 0)}</div>
    ...
    <div className="min-h-screen bg-[radial-gradient(1200px_600px_at_10%_-10%,rgba(59,130,246,0.15),transparent),radial-gradient(1200px_600px_at_90%_10%,rgba(16,185,129,0.12),transparent)] bg-slate-950">
      <div className="max-w-7xl mx-auto px-4 md:px-8 py-8">
        <header className="mb-6">
          ...
          <p className="text-white/70 text-sm md:text-base mt-1">Real-time parking availability and reservation system</p>
        </header>

        <div className="text-center mt-2 text-white text-2xl font-semibold">Choose Your Location</div>
        ...
        <section className="mt-8 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          {zones.map((z) => {
            const occupied = Math.max(z.total - z.available, 0);

            // build a query-safe list of features
            const featuresQuery = encodeURIComponent(z.features.join(","));
        ...
        {zones.length === 0 && <div className="text-center text-gray-300 mt-6">No zones found</div>}
      </div>
    </div>
  );
};
