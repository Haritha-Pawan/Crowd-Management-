// src/Counter/AddCounter.jsx  (use any path you like)
...
const AddCounter = ({ isOpen, onClose, onCreate }) => {
  const [form, setForm] = useState({
    name: "",
    entrance: "",
    status: "Entry",   // route allows Entry | Exit
    capacity: "",
    load: "",
    staff: "",
    isActive: true,
  ...
  const submit = async (e) => {
    ...
    try {
      const { data } = await axios.post(`${API}/counter`, payload);
      onCreate?.(data.counter);         // backend returns { counter }
      onClose?.();
      setForm({
        name: "",
        entrance: "",
  ...
  console.error("POST /api/counter failed:", data || err);
  alert(msg);
}
  ...
  return (
    <div className="fixed z-50 inset-0 flex justify-center items-center bg-black/60">
      <div className="w-[90%] max-w-2xl bg-[#0f172a] p-5 rounded-md border border-white/10">
        <h2 className="text-white font-bold text-xl">Add New Counter</h2>
        <h6 className="text-gray-300 font-bold text-xs">Register a new entry/exit counter</h6>
